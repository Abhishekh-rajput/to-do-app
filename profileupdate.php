...
$qt = date('d-m-Y H:i');
$query=mysqli_query($bd,"UPDATE `userdata` SET `FullName` = '$fn', `UserMobileNumber` = '$contactno', `UserEmail` = '$email', `AddressType` = '$hometype', `House` = '$home',`Zip` = '$zip',`State` = '$state' WHERE `userdata`.`UserName` ='$uid' ") or die(mysqli_error($bd));
